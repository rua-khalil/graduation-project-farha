...
class UserHomePage extends StatelessWidget {
  ...
  const UserHomePage({super.key, this.userName = 'User'});
            ...
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // ── Hero Section ─────────────────
                  UserHomeHeroSection(userName: userName),
...
// ── User Hero Section ─────────────────────────────────
class UserHomeHeroSection extends StatelessWidget {
  final String userName;
  const UserHomeHeroSection({super.key, required this.userName});
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── Description ──────────────────────
            const Text(
              'Browse venues, manage your bookings, and track your favorites — all from one place.',
              style: TextStyle(
              ...
              children: [
                ...
                ElevatedButton(
                  ...
                  child: const Row(
                    ...
                    children: [
                      Text('Explore Venues', style: TextStyle(fontWeight: FontWeight.w700)),
                      SizedBox(width: 6),
                ...
                ElevatedButton(
                  ...
                  child: const Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text('My Dashboard', style: TextStyle(fontWeight: FontWeight.w700)),
                    ],
                  ),
                ),
              ],
          ...
          data: Theme.of(context).copyWith(
            ...
            textButtonTheme: TextButtonThemeData(
              style: TextButton.styleFrom(
                foregroundColor: Color(0xFFD5A217),
              ),
            ),
          ),
          child: child!,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        final RenderBox box =
                        _guestKey.currentContext!.findRenderObject()
                        as RenderBox;
                        final Offset offset =
                          ...
                          children: [
                            Expanded(
                              child: Text(
                                ...
                                style: TextStyle(
                                  color: _selectedGuests == null ? kNavy : kNavy,
                                  fontSize: 12,
                                  fontWeight: FontWeight.w600,
                                ),
                              ),
    ...
    return Padding(
      ...
      child: FutureBuilder<HomeStats>(
        ...
        builder: (context, snapshot) {
          // أثناء التحميل أو لو فشل: منعرض قيم افتراضية بدل فراغ
...
class HomeCategoryItem extends StatelessWidget {
  ...
  const HomeCategoryItem({super.key, required this.icon, required this.title, required this.count, this.subtitle});
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: ElevatedButton(
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const BrowsePage())),
              style: ElevatedButton.styleFrom(
                elevation: 0, backgroundColor: Colors.white, foregroundColor: kNavy,
                  ...
                  children: [
                    for (int i = 0; i < venues.length; i++) ...[
                      VenueCard(
                        ...
                        rating: venues[i].rating.toString(),
                        reviews: venues[i].reviews.toString(),
              ...
              Positioned(
                ...
                child: Row(
                  children: [
                    ...
                    _Tag(label: category, color: kNavy),
                  ],
                ),
              ),
            ],
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(child: Text(name, style: const TextStyle(color: kNavy, fontWeight: FontWeight.w800, fontSize: 16))),
                    const Icon(Icons.star_rounded, color: kGold, size: 16),
                    const SizedBox(width: 4),
                    Text('$rating ($reviews)', style: const TextStyle(color: kGray, fontSize: 12, fontWeight: FontWeight.w600)),
                  ],
                ),
                const SizedBox(height: 10),
                Row(children: [const Icon(Icons.location_on_outlined, color: kGray, size: 15), const SizedBox(width: 6), Text(location, style: const TextStyle(color: kGray, fontSize: 13, fontWeight: FontWeight.w500))]),
                const SizedBox(height: 6),
                Row(children: [const Icon(Icons.people_alt_outlined, color: kGray, size: 15), const SizedBox(width: 6), Text(capacity, style: const TextStyle(color: kGray, fontSize: 13, fontWeight: FontWeight.w500))]),
                const SizedBox(height: 14),
                const Divider(color: kBorder, thickness: 1, height: 20),
                Row(
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(price, style: const TextStyle(color: kNavy, fontWeight: FontWeight.w900, fontSize: 16)),
                      ],
                    ),
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => VenueDetailPage(venueId: id),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: kGold, foregroundColor: kWhite, elevation: 0,
  ...
  Widget _numberedValueCard(int number, IconData icon, String title, String description) {
    return FarhaCard(
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(number.toString().padLeft(2, '0'), style: const TextStyle(color: kGray, fontSize: 13, fontWeight: FontWeight.w700)),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const FarhaSectionHeader(title: 'How it works', subtitle: 'A simple flow from discovery to booking.'),
          const SizedBox(height: 20),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 16),
            child: Column(
              children: [
                _numberedValueCard(1, Icons.search, 'Browse & compare', 'Explore venues and services with clean details, ratings, and pricing.'),
                const SizedBox(height: 14),
                _numberedValueCard(2, Icons.favorite_border, 'Save favorites', 'Keep your preferred options in one place and return anytime.'),
                const SizedBox(height: 14),
                _numberedValueCard(3, Icons.verified_user_outlined, 'Book with confidence', 'Send your booking request and manage everything from your dashboard.'),
              ],
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: const [
          FarhaSectionHeader(title: 'What our clients say', subtitle: 'Real experiences from couples who used Farha.'),
          SizedBox(height: 20),
          Padding(
            padding: EdgeInsets.symmetric(horizontal: 16),
            child: Column(
              children: [
                TestimonialCard(text: 'Farha made finding our dream venue so easy. The whole experience felt clean, modern, and well organized.', name: 'Sarah & Michael'),
                SizedBox(height: 14),
                TestimonialCard(text: 'We loved how simple it was to compare options and move from browsing to booking without confusion.', name: 'Ahmed & Fatima'),
                SizedBox(height: 14),
                TestimonialCard(text: 'The platform saved us a lot of time and gave us confidence in every decision we made.', name: 'David & Emily'),
              ],
            ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    return FarhaCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: List.generate(5, (index) => const Icon(Icons.star_rounded, color: Color(0xFFD5A217), size: 18))),
          const SizedBox(height: 10),
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Ready to start planning your event?', style: TextStyle(color: kWhite, fontSize: 25, height: 1.2, fontWeight: FontWeight.w900)),
            const SizedBox(height: 10),
            const Text('Join Farha and discover venues and vendors with a premium, organized workflow.', style: TextStyle(color: Colors.white70, fontSize: 14, height: 1.6)),
            const SizedBox(height: 18),
            Row(
              children: [
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(elevation: 0, backgroundColor: kGold, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(elevation: 0, backgroundColor: Colors.white, foregroundColor: kNavy, padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
