...
class VendorChatMessage {
  ...
  factory VendorChatMessage.fromJson(Map<String, dynamic> json) {
...
class VendorMessagesPage extends StatefulWidget {
  const VendorMessagesPage({super.key});

  @override
  State<VendorMessagesPage> createState() => _VendorMessagesPageState();
}

class _VendorMessagesPageState extends State<VendorMessagesPage> {
  bool _loading = true;
  String? _error;
  List<VendorMessageModel> _messages = [];
  String _search = '';
  ...
  Future<void> _fetchMessages() async {
    ...
    try {
      ...
      // final response = await http.get(Uri.parse('https://your-api.com/vendor/messages'));
      // final data = jsonDecode(response.body) as List;
      // _messages = data.map((e) => VendorMessageModel.fromJson(e)).toList();
  ...
  List<VendorMessageModel> get _filtered {
    if (_search.isEmpty) return _messages;
    return _messages
        .where((m) => m.customerName.toLowerCase().contains(_search.toLowerCase()))
        .toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: kBg,
      appBar: const VendorNav(activeTab: 'Messages'),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Stay connected with your clients and manage conversations..',
...
// ── Conversation Card ─────────────────────────────────────────────────────────
class _VendorConversationCard extends StatelessWidget {
  final VendorMessageModel message;
  final VoidCallback onTap;

  const _VendorConversationCard({required this.message, required this.onTap});

  String get _initials {
    final parts = message.customerName.trim().split(' ');
    if (parts.length >= 2) return '${parts[0][0]}${parts[1][0]}'.toUpperCase();
    return message.customerName.isNotEmpty ? message.customerName[0].toUpperCase() : '?';
  }
...
class _VendorChatPageState extends State<VendorChatPage> {
  bool _loading = true;
  List<VendorChatMessage> _chatMessages = [];
  ...
  String get _initials {
    ...
    if (parts.length >= 2) return '${parts[0][0]}${parts[1][0]}'.toUpperCase();
    return widget.customerName.isNotEmpty ? widget.customerName[0].toUpperCase() : '?';
  }
  ...
  Future<void> _fetchChat() async {
    ...
    try {
      // TODO: Replace with your real API endpoint
      // final response = await http.get(Uri.parse('https://your-api.com/vendor/messages/${widget.conversationId}'));
      // final data = jsonDecode(response.body) as List;
      // _chatMessages = data.map((e) => VendorChatMessage.fromJson(e)).toList();
  ...
  void _sendMessage() {
    ...
    // TODO: Send to API
    // await http.post(Uri.parse('https://your-api.com/vendor/messages/${widget.conversationId}'), body: {'text': text});

    setState(() {
      _chatMessages.add(VendorChatMessage(
        text: text,
        isMe: true,
        time: _nowTime(),
      ));
          ...
          Expanded(
            child: _loading
              ...
              itemBuilder: (context, index) {
                return _VendorChatBubble(message: _chatMessages[index]);
              },
            ),
          ),
...
// ── Chat Bubble ───────────────────────────────────────────────────────────────
class _VendorChatBubble extends StatelessWidget {
  final VendorChatMessage message;
  const _VendorChatBubble({required this.message});
